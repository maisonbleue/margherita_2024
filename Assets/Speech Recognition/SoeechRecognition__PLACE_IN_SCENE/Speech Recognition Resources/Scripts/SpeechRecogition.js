...
var onUpdateListeningEventHandler = function(eventArgs) {
    ...
    if (eventArgs.isFinalTranscription) {
        ...
        script.openAIScript.callOpenAI(eventArgs.transcription);
        if (script.debug) {
            print("Voice ML detected: " + eventArgs.transcription);
        }
...
script.voiceStop = function() {
    script.vmlModule.stopListening();
}

script.voiceStart = function() {
    script.vmlModule.startListening(options);
}